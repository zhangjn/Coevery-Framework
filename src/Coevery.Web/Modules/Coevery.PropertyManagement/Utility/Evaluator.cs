...
namespace Coevery.PropertyManagement.Utility
{
    public class Evaluator
    {
        ...
        public Evaluator(Type returnType, string expression, string name, IEnumerable<ParameterEntity> parasTypes)
        {
            //创建可执行字符串数组 
            EvaluatorItem[] items = { new EvaluatorItem(returnType, expression, name, parasTypes) };
            ConstructEvaluator(items); //调用解析字符串构造函数进行解析 
        }
        ...
        private void ConstructEvaluator(EvaluatorItem[] items)
        {
            ...
            */
            ...
            foreach (EvaluatorItem item in items) //遍历每一个可执行字符串项 
            {
                ...
                {
                    paraStr.AppendFormat("{0} {1},", para.ParaType.Name, para.ParaName);
                }

                code.AppendFormat(" public {0} {1}({2}) ", //添加定义公共函数代码 
                item.ReturnType.Name, //函数返回值为可执行字符串项中定义的返回值类型 
                item.Name, paraStr.ToString().TrimEnd(',')); //函数名称为可执行字符串项中定义的执行字符串名称 
        ...
        /// <param name="name">执行字符串名称</param> 
        /// <returns>执行结果</returns> 
        public object Evaluate(string name, IEnumerable<ParameterEntity> paras = null)
        {
            MethodInfo mi = _Compiled.GetType().GetMethod(name);//获取 _Compiled 所属类型中名称为 name 的方法的引用 
            return mi.Invoke(_Compiled, paras.Select(it => it.ParaValue).ToArray()); //执行 mi 所引用的方法 
        }
        #endregion
        ...
        /// </summary>
        /// <param name="code">要执行的表达式</param>
        /// <param name="parasTypes">表达式里面的参数</param>
        /// <returns></returns>
        static public object EvaluateToObject(string code, IEnumerable<ParameterEntity> parasTypes)
        {
            Evaluator eval = new Evaluator(typeof(object), code, staticMethodName, parasTypes);//生成 Evaluator 类的对像 
            return eval.Evaluate(staticMethodName, parasTypes); //执行并返回 object 型数据 
        }
        ...
        {
            var eval = new Evaluator(typeof(double), code, staticMethodName);//生成 Evaluator 类的对像 
            return (double)eval.Evaluate(staticMethodName); //执行并返回 
        }
        #endregion
    ...
    public class EvaluatorItem
    {
        ...
        public IEnumerable<ParameterEntity> Paras;
        /// <summary> 
        /// 可执行字符串项构造函数 
        /// </summary> 
        /// <param name="returnType">返回值类型</param> 
        /// <param name="expression">执行表达式</param> 
        /// <param name="name">执行字符串名称</param> 
        public EvaluatorItem(Type returnType, string expression, string name, IEnumerable<ParameterEntity> paras = null)
        {
