...
{
    [Themed]
    public class BillReportController : Controller, IUpdateModel
    {
        ...
        public BillReportController(ICoeveryServices services,
        ...
        public ActionResult List(FormCollection input, int page = 1, int pageSize = 10, string sortBy = null, string sortOrder = "asc") {
            ...
            var list = _reportServices.GenerateBillReportListViewModels(query).ToList();
            decimal? totalReceivedMoney = list.Sum(l => l.ChargeItemAmount);

            int totalRecords = list.Count;
            list = list.OrderBy(sortBy, sortOrder).Skip((page - 1)*pageSize).Take(pageSize).ToList();
            var result = new {
                page,
        ...
        [HttpPost]
        public ActionResult BillReportSummary(FormCollection input, int page = 1, int pageSize = 10, string sortBy = null, string sortOrder = "asc") {
            if (!Services.Authorizer.Authorize(Permissions.BillManage, T("没有账单查看权限")))
                return new HttpUnauthorizedResult();

            var query = QueryBillRecord(input).OrderBy(sortBy, sortOrder);
            var list = _reportServices.GenerateBillReportSummaryViewModel(query.ToList()).ToList();
        ...
        public ActionResult ExportExcel(FormCollection input, string gridColumns, string gridSummaryColumns) {
            ...
            var sheet = workbook.CreateSheet("账单明细表");
            ...
            using (var exportData = new MemoryStream()) {
                ...
                string saveAsFileName = string.Format("{0}-{1:d}.xls", "账单明细表", DateTime.Now).Replace("/", "-");
