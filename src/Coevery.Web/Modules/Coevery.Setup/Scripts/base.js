(function ($) {
    ...
    $.extend({
        Coevery: {
            __cookieName: "Orchrd", // Coevery, on a diet
            ...
            cookie: function (scope, value, options) { // a light-weight wrapper around $.cookie for an Coevery.* cookie name
                // because some browsers are weird about the path
                if (options && options.path && $.Coevery.__doCookiePathHack(options.path)) {
                    options.path = options.path.substring(0, options.path.lastIndexOf("/"));
                }
                return $.cookie($.Coevery.__cookieName + (scope ? scope.toLowerCase() : ""), value, options);
            },
            cookiesInTheCoevery: function () {
                return $.Coevery.cookiesLike($.Coevery.__cookieName);
            },
            ...
            setting: function (name, value, options) { // cookie-stored settings (only, at the moment)
                ...
                if (typeof value === "undefined") { // try to get the setting value from the default "root" cookie
                    var cookies = $.Coevery.cookiesInTheCoevery();
                    for (var i = 0; i < cookies.length; i++) {
                    ...
                    var cookie = $.Coevery.cookie(scope);
                    ...
                    $.Coevery.cookie(scope, dataString, { expires: $.Coevery.__cookieExpiration, path: (options && options.path) || "/" }); // todo: default path should be app path
                }
            }
        }
    });
