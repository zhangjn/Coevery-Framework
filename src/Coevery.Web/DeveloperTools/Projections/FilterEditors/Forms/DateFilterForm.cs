...
namespace Coevery.DeveloperTools.Projections.FilterEditors.Forms {
    public class DateFilterForm : IFormProvider {
        public const string FormName = "DateFilter";

        protected dynamic Shape { get; set; }
        public Localizer T { get; set; }

        public DateFilterForm(IShapeFactory shapeFactory) {
            Shape = shapeFactory;
        ...
        public void Describe(DescribeContext context) {
            Func<IShapeFactory, object> form =
                shape => {
                    ...
                    var f = Shape.FilterEditors_DateFilter(
                        Id: FormName,
                        Operators: operators
                        );
                    return f;
