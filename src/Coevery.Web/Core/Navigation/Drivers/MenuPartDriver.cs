...
namespace Coevery.Core.Navigation.Drivers {
    ...
    public class MenuPartDriver : ContentPartDriver<MenuPart> {
        ...
        protected override DriverResult Editor(MenuPart part, string displayType, dynamic shapeHelper)
        {
        ...
        protected override DriverResult Editor(MenuPart part, IUpdateModel updater, string displayType, dynamic shapeHelper)
        {
            if (!_authorizationService.TryCheckAccess(Permissions.ManageMainMenu, _coeveryServices.WorkContext.CurrentUser, part))
                return null;
