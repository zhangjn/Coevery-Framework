...
namespace Coevery.Core.Navigation.Drivers {
    ...
    public class MenuItemPartDriver : ContentPartDriver<MenuItemPart> {
        ...
        protected override DriverResult Editor(MenuItemPart part ,string displayType,dynamic shapeHelper) {
        ...
        protected override DriverResult Editor(MenuItemPart part, IUpdateModel updater, string displayType, dynamic shapeHelper)
        {
            var currentUser = _workContextAccessor.GetContext().CurrentUser;
            if (!_authorizationService.TryCheckAccess(Permissions.ManageMainMenu, currentUser, part))
                return null;
