...
namespace Coevery.Environment {
    // All the event handlers that DefaultCoeveryHost implements have to be declared in CoeveryStarter
    public class DefaultCoeveryHost : ICoeveryHost, IShellSettingsManagerEventHandler, IShellDescriptorManagerEventHandler {
        ...
        void ICoeveryHost.Initialize() {
            Logger.Information("Initializing");
            BuildCurrent();
            Logger.Information("Initialized");
        }

        void ICoeveryHost.ReloadExtensions() {
            DisposeShellContext();
        }

        void ICoeveryHost.BeginRequest() {
            Logger.Debug("BeginRequest");
            BeginRequest();
        }

        void ICoeveryHost.EndRequest() {
            Logger.Debug("EndRequest");
            EndRequest();
        }

        IWorkContextScope ICoeveryHost.CreateStandaloneEnvironment(ShellSettings shellSettings) {
        ...
        private void MonitorExtensions() {
            ...
            _cacheManager.Get("CoeveryHost_Extensions",
                              ctx => {
