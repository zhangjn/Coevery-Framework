...
namespace Coevery.Environment.ShellBuilders {
    ...
    public class CompositionStrategy : ICompositionStrategy {
        ...
        public ShellBlueprint Compose(ShellSettings settings, ShellDescriptor descriptor) {
            ...
            if (descriptor.Features.Any(feature => feature.Name == "Coevery.Framework"))
        ...
        private static IEnumerable<string> GetExcludedTypes(IEnumerable<Feature> features) {
            ...
            foreach (Feature feature in features) {
                foreach (Type type in feature.ExportedTypes) {
                    foreach (CoeverySuppressDependencyAttribute replacedType in type.GetCustomAttributes(typeof(CoeverySuppressDependencyAttribute), false)) {
        ...
        private static IEnumerable<Feature> BuiltinFeatures() {
            yield return new Feature {
                ...
                ExportedTypes =
                    typeof(CoeveryStarter).Assembly.GetExportedTypes()
                    .Where(t => t.IsClass && !t.IsAbstract)
                    .Except(new[] { typeof(DefaultCoeveryHost) })
                    .ToArray()
            };
        }
