...
namespace Coevery.Data {
    public class SessionLocator : ISessionLocator, ITransactionManager, IDisposable {
        ...
        private void EnsureSession() {
            ...
            _session = sessionFactory.OpenSession(new CoeverySessionInterceptor(_interceptors.ToArray(), Logger));
        }

        class CoeverySessionInterceptor : IInterceptor {
            private readonly ISessionInterceptor[] _interceptors;
            private readonly ILogger _logger;

            public CoeverySessionInterceptor(ISessionInterceptor[] interceptors, ILogger logger) {
                _interceptors = interceptors;
                _logger = logger;
            }
